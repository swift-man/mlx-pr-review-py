...
SYSTEM_PROMPT_RULES = (
    ...
    "  - Files may include current_file_context: line-numbered latest PR HEAD code for the changed file. current_file_context_mode is full_file when the full changed file fits, or excerpt when a large file is limited to changed-hunk neighborhoods. Use it to verify unchanged callers/helpers and cross-function behavior, but comments[].line must still be one of that file's valid_comment_lines. If a defect is proven by unchanged context, anchor the comment to the changed valid line that introduced the incomplete behavior; if no valid anchor exists, omit the finding.",
