...
SYSTEM_PROMPT_RULES = (
    ...
    "Do not put structural changes the diff performs (type change, file addition or removal, new function or class, added import, field rename, comment translation) into concerns as mere narration. A structural change is not a concern unless you can point to a concrete bug, risk, or missing validation it introduces.",
    "Concerns must be stated as problems, not as facts. Sentences that end with Korean narration suffixes such as '추가되었습니다', '변경되었습니다', '수정되었습니다' are descriptive - not concerns. Either rewrite them as a real risk statement or drop them entirely before producing the JSON.",
...
USER_PROMPT_RULES = (
    ...
    "concerns 에 diff 가 수행한 구조적 변경(파일 추가/삭제, 타입 변경, 함수/클래스 추가, import 추가, 주석 번역, 필드 rename 등) 그 자체를 서술로 적지 마세요. 구조 변경은 그 자체로는 concern 이 아니며, 그 변경 때문에 발생하는 구체적 버그·리스크·누락 검증이 있을 때만 그 위험을 적습니다.",
    "예를 들어 '주석이 한국어로 수정되었습니다', '새로운 테스트 파일이 추가되었습니다', '필드 타입이 MappingProxyType 으로 변경되었습니다' 는 모두 서술이지 concern 이 아닙니다. 이런 문장은 concerns 에서 제외하세요.",
    "concern 은 '~때문에 ~할 수 있습니다 / ~가 누락돼 있습니다 / ~가 필요합니다 / ~할 위험이 있습니다' 처럼 문제 진술 형태여야 합니다. '~되었습니다' 로 끝나는 사실 서술만 있다면 concern 이 아닙니다.",
